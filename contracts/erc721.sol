...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function deleteTokenId(address myaddr,uint256 tokenId) internal {
        require(tokenId != 0, "zero address");
        uint index = 0;
        for(uint i=0; i<_myTokenIds[myaddr].length; i++){
            if(_myTokenIds[myaddr][i] == tokenId){
                index = i;
                break;
            }
        }
        require(index>=0 && index<_myTokenIds[myaddr].length, "index out of range");
        if(index == _myTokenIds[myaddr].length-1){
            _myTokenIds[myaddr].pop();
        }else{
            uint256 lastElement = _myTokenIds[myaddr][_myTokenIds[myaddr].length - 1];
            _myTokenIds[myaddr][index] = lastElement;
            _myTokenIds[myaddr].pop();
        }
    }
    ...
    function getMyTokens(address owner) public view virtual returns (uint256[] memory) {
        require(owner != address(0), "ERC721: balance query for the zero address");
